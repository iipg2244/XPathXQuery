...
namespace XPathXQuery.Models
{
    public class Repository
    {
        ...
        public static string GetFullPathApp()
        {
            return AppDomain.CurrentDomain.BaseDirectory.Substring(0, AppDomain.CurrentDomain.BaseDirectory.Length - 1).Replace(@"\", "/");
        }
        ...
        public static string? SelectPath(string title = "")
        {
            ...
            if (openFileDialog.ShowDialog() == true)
                return openFileDialog.SelectedPath.Replace(@"\", "/");
            return null;
        }
        ...
        public static List<T> FromIListToList<T>(System.Collections.IList iList)
        {
            return iList.Cast<T>().ToList();
        }
        ...
        public List<Movie> AllMovie(XmlNodeList? nodeList, List<Movie> lmovies)
        {
            if (nodeList?.Count > 0)
            {
                foreach (XmlNode node in nodeList)
                {
                    ...
                    if (movie != null)
                    {
                        XmlNodeList? nodeList2 = docMovies.SelectNodes($"//movie[./_id = '{movie.Id}']/actors");
                        if (nodeList2?.Count > 0)
                        {
                            foreach (XmlNode node2 in nodeList2)
                            {
                                Actor? actor = Deserialize<Actor>(node2);
                                if (actor != null)
                                {
                        ...
                        XmlNodeList? nodeList3 = docMovies.SelectNodes($"//movie[./_id = '{movie.Id}']/directors");
                        if (nodeList3?.Count > 0)
                        {
                            foreach (XmlNode node3 in nodeList3)
                            {
                                Director? director = Deserialize<Director>(node3);
                                if (director != null)
                                {
        ...
        public List<Movie> AllMovie(List<Person> lpeople)
        {
            ...
            {
                XmlNodeList? nodeList = docMovies.SelectNodes($"//movie[actors/id = '{person.Id}']");
                lmovies = AllMovie(nodeList, lmovies);
                nodeList = docMovies.SelectNodes($"//movie[directors/id = '{person.Id}']");
                lmovies = AllMovie(nodeList, lmovies);
            }
            lmovies = lmovies.OrderBy(x => x.Name).ToList();
            return lmovies;
